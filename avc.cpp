...
int Robot::MeasureLine(){ //only coded for quad 2 rn
	...
	clock_gettime(CLOCK_MONOTONIC, &ts_start);
		...
		for(int countCol = 0; countCol < 320; countCol++){
			...
			if(vertLineCount > 40 && (err < 1 || err > -1) && lineCount < 180 && (prevErr < 1 && prevErr > -1)){
			corner = 0;	
			printf("Corner: %d",corner);
			}
			...
			if (lineCount < 20 && vertLineCount < 20 && err == 0 && prevErr < 1 && corner == 0){
				...
				sleep1(1000);
					...
					return 0;
			} else if ((lineCount >= 230 && (err < 1 || err > 5.25) && (lineCount - prevLineCount) < 50 && xroadBool == 1 && corner == 0 && (err - prevErr < 1 && err - prevErr > -1)) || (lineCount > 270 && corner == 0) || lineCount > 300){
